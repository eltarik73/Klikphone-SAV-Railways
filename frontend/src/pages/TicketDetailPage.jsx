          ...
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-slate-100 shrink-0 animate-pulse" />
          ...
          <div className="card p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              {editingPricing && (
                <div className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => { clearTimeout(pricingTimerRef.current); setEditingPricing(false); setAutoSavePricing(null); setPricingForm({ devis_estime: t.devis_estime || '', tarif_final: t.tarif_final || '', acompte: t.acompte || '', technicien_assigne: t.technicien_assigne || '', type_ecran: t.type_ecran || '', date_recuperation: t.date_recuperation || '', reduction_montant: t.reduction_montant || '', reduction_pourcentage: t.reduction_pourcentage || '', commande_piece: t.commande_piece || 0, type_document: t.type_document || 'devis' }); setRepairLines(parseRepairLines(t)); }} className="btn-ghost text-xs px-2.5 py-1.5"><X className="w-3.5 h-3.5" /> Fermer</button>
                </div>
              )}
            </div>
            ...
            {editingPricing ? (
              ...
              <div className="space-y-4">
                ...
                {/* 3. Séparateur tarification + toggle Devis/Facture */}
                ...
                <div className="flex items-center gap-2">
                  <label className="input-label mb-0 flex items-center gap-1"><FileText className="w-3 h-3" /> Type de document</label>
                  <div className="flex bg-slate-100 rounded-lg p-0.5 text-[11px] font-semibold">
                    <button type="button" onClick={() => updatePricingForm({ type_document: 'devis' })}
                      className={`px-3 py-1 rounded-md transition-all ${(pricingForm.type_document || 'devis') === 'devis' ? 'bg-white shadow-sm text-brand-700' : 'text-slate-400'}`}>Devis</button>
                    <button type="button" onClick={() => updatePricingForm({ type_document: 'facture' })}
                      className={`px-3 py-1 rounded-md transition-all ${pricingForm.type_document === 'facture' ? 'bg-white shadow-sm text-brand-700' : 'text-slate-400'}`}>Facture</button>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="mb-3 flex items-center gap-2">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${
                    (t.type_document || 'devis') === 'facture' ? 'bg-brand-100 text-brand-700' : 'bg-slate-100 text-slate-500'
                  }`}>
                    <FileText className="w-3 h-3 inline mr-0.5" />{(t.type_document || 'devis') === 'facture' ? 'Facture' : 'Devis'}
                  </span>
                </div>
