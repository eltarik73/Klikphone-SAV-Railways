...
import {
  LogOut, LayoutDashboard, Plus, Search,
  Menu, X, Smartphone,
} from 'lucide-react';
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-sidebar border-b border-white/10 flex items-center px-4 z-40">
        ...
        <div className="flex items-center gap-2.5 ml-2">
          ...
          <span className="text-white font-bold tracking-tight">KLIKPHONE</span>
        </div>
      </div>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 bottom-0 w-64 bg-sidebar flex flex-col z-50
        transform transition-transform duration-300 ease-in-out
        ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}
        lg:translate-x-0
      `}>
        {/* Logo */}
        <div className="h-14 px-5 flex items-center gap-3 border-b border-white/[0.06] shrink-0">
