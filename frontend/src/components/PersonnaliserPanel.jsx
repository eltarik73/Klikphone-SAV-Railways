import { X, Timer, Clock, ListOrdered, RotateCcw, Info, Shuffle } from 'lucide-react';
...
function ToggleSwitch({ icon: Icon, label, desc, value, onChange, activeColor }) {
  const bg = value ? (activeColor || 'bg-emerald-500') : 'bg-slate-300';
  return (
    <div className="flex items-center justify-between py-3 border-b border-slate-100 last:border-b-0">
      ...
      <button
        onClick={onChange}
        className={`relative w-11 h-6 rounded-full transition-colors duration-200 shrink-0 ml-3 ${bg}`}
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-4 border-b border-slate-100">
            ...
            <ToggleSwitch
              icon={Shuffle}
              label="Mode édition"
              desc="Déplacer les widgets entre colonnes"
              value={editMode}
              onChange={() => { onToggleEditMode(); onClose(); }}
              activeColor="bg-blue-500"
            />
            {editMode && (
              <div className="mt-2 p-3 rounded-lg bg-blue-50 border border-blue-200">
                <div className="text-xs font-semibold text-blue-600">Mode édition activé</div>
                <div className="text-[11px] text-slate-500 mt-0.5">
                  Glisse les widgets sur la page. Désactive quand c'est bon.
                </div>
              </div>
            )}
          </div>
